...
export default {
    ...
    created() {
        let _this = this;
        if (!_this.opAttr) return
        document.addEventListener('click', (e) => {
            // 初始化页面中带此特性标记的图片
            if (e.target.localName === 'img' && e.target.attributes[_this.opAttr]) {
                console.log(e.target.attributes['data-src'])
                let src = e.target.attributes['data-src'] ? e.target.attributes['data-src'].value : e.target.attributes.src.value
                let srcOpt = {
                    msrc: src,
                    src,
                }
                if (_this.pList.length === 0)
                    _this.pList.push(srcOpt)
                else
                    _this.pList = [srcOpt]
                _this.show(0)
            }
    ...
    methods: {
        ...
        doInit(index) {
            ...
            this.photoswipe.listen('gettingData', (i, e) => {
